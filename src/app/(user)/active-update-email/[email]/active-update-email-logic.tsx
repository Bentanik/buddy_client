...
import { Data, ErrorResponse } from "@/stores/activeAccountSlice";
...
export default function ActiveUpdateEmailLogic({ children, activeAccountEmail }: ActiveAccountProps) {
    ...
    const fetchActiveAccount = async (data: Data) => {
        try {
            const res = await dispatch(activeUpdateEmailThunk(data)).unwrap();
            return toast.custom(
        ...
        <div>
            ...
            <section>
                <Backdrop
                    ...
                    open={userProfileState?.profilePrivate?.statusActiveUpdateEmail === "loading"}
                >
